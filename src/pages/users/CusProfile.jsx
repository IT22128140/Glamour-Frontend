...
const Profile = () => {
    ...
    useEffect(() => {
        if (userID) {
            axios.get(`http://localhost:3000/measurements/user/${userID}`)
                .then((response) => {
                    console.log(response.data);
                    if (response.data) {
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col items-center">
            ...
            <div className="mt-8 w-full max-w-4xl">
                <div className="flex flex-col items-center">
                    <div className="w-32 h-32 rounded-full overflow-hidden mb-4">
                        <img src="emp.png" alt="Profile" className="object-cover w-full h-full" />
                    </div>
                    <div className="bg-white shadow-lg rounded-lg w-full p-8 mb-8">
                        <h1 className="text-3xl font-Aboreto font-bold text-primary mb-6 text-center">Profile Details</h1>
            ...
            {loadingMeasurements ? (
                ...
                <div className="w-full max-w-4xl bg-white shadow-lg rounded-lg p-8 mb-8">
                    <h2 className="text-3xl font-bold text-primary text-center mb-8">Body Measurements</h2>
                    <div className="grid grid-cols-2 gap-6">
                        <div className="flex items-center gap-4 p-4 space-x-4">
                            <p className="text-lg font-semibold text-gray-600">Measurement ID:</p>
                        ...
                        <div className="flex items-center gap-4 p-4 space-x-4">
                            <p className="text-lg font-semibold text-gray-600">Bust Size:</p>
                            <p>{measurements.Bust}</p>
                        </div>
                        <div className="flex items-center gap-4 p-4 space-x-4">
                            <p className="text-lg font-semibold text-gray-600">Waist Size:</p>
                        ...
                        <div className="flex items-center gap-4 p-4 space-x-4">
                            <p className="text-lg font-semibold text-gray-600">Shoulder Width:</p>
                ...
                {measurements?.Gender === 'Female' && (
                    <>
                        <h2 className="text-2xl font-bold text-primary w-full text-center mb-4">
                            Your model size for below cloth types: {measurements.TopSize || "N/A"}
                        </h2>
                        <div className="flex justify-center space-x-4">
                            {femaleTops.map((image, index) => (
                                <img key={index} src={image} alt="Female Top" className="w-40 rounded-lg shadow-md" />
                            ))}
                        </div>
                        <h2 className="text-2xl font-bold text-primary w-full text-center mt-10 mb-4">
                            Your model size for below cloth types: {measurements.PantSize || "N/A"}
                        </h2>
                        <div className="flex justify-center space-x-4">
                            {femalePants.map((image, index) => (
                                <img key={index} src={image} alt="Female Pants" className="w-40 rounded-lg shadow-md" />
                            ))}
                        </div>
                    </>
                )}
                    ...
                    <>
                        ...
                        <div className="flex justify-center space-x-4">
                            {maleShirts.map((image, index) => (
                                <img key={index} src={image} alt="Male Shirt" className="w-40 rounded-lg shadow-md" />
                            ))}
                        </div>
                        <h2 className="text-2xl font-bold text-primary w-full text-center mt-10 mb-4">
                            Your model size for below cloth types: {measurements.TopSize || "N/A"}
                        ...
                        <div className="flex justify-center space-x-4">
                            {maleTrousers.map((image, index) => (
                                <img key={index} src={image} alt="Male Pants" className="w-40 rounded-lg shadow-md" />
                            ))}
                        </div>
                    </>
                )}
